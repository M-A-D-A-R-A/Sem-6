...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        corners = list(state[1])  # stores list of corners
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            rem_corners = set(corners)
            if next in rem_corners:
                rem_corners.remove(next)

            # adding successor state to successors list
            if not hitsWall:
                successors.append(
                    (((nextx, nexty), tuple(rem_corners)), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    position = state[0]  # stores current search state
    corners = list(state[1])  # stores list of all corners
    ...
    while len(corners) > 0:
        ...
        # finding closest corner from all 4
        for i in range(0, len(corners)):
            # manhattan distance from current position to corners
            manhattan = util.manhattanDistance(position, corners[i])
            if manhattan < cornerDistance:
                closestCorner = corners[i]
                cornerDistance = manhattan
        position = closestCorner
        heuristic += cornerDistance
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    foods = foodGrid.asList()  # storing list of foods
    if len(foods) == 0:
        return 0

    heuristic = 0
    # mazeDistance finds the maze distance between any two points
    farFoodDist = mazeDistance(
        position, foods[0], problem.startingGameState)

    for food in foods:
        foodDist = mazeDistance(position, food, problem.startingGameState)
        if foodDist > farFoodDist:
            farFoodDist = foodDist
    heuristic = farFoodDist
    return heuristic
  #  return 0
